...
{
    public class EdPropertySet2x3 : BasePropertySetDefinition
    {
        private IfcPropertySet ifcPropertySet;

        public EdPropertySet2x3(IIfcObjectDefinition ifcObjectDefinition, IfcPropertySet ifcPropertySet, ModelIFC modelIFC, ModelItemIFCObject modelItem) : base(ifcObjectDefinition, ifcPropertySet, modelIFC, modelItem)
        {
        ...
        protected override IEnumerable<IPropertyModel<IIfcResourceObjectSelect>> FillCollectionProperty()
        {
            ...
            foreach (IIfcProperty property in ifcPropertySet.HasProperties.Where(it => it is IIfcPropertySingleValue || it is IIfcPropertyReferenceValue))
            {
                ...
                yield return new EditorProperty2x3<IIfcProperty>(property, ModelIFC, this);
            }
            //return CollectionProperty;
        }
    }

    public class EdElementQuantity2x3 : BasePropertySetDefinition
    {
        private IfcElementQuantity ElementQuantity;

        public EdElementQuantity2x3(IIfcObjectDefinition ifcObject, IfcElementQuantity ElementQuantity, ModelIFC modelIFC, ModelItemIFCObject modelItem) : base(ifcObject, ElementQuantity, modelIFC, modelItem)
        {
        ...
        protected override IEnumerable<IPropertyModel<IIfcResourceObjectSelect>> FillCollectionProperty()
        {
            ...
            foreach (IIfcPhysicalQuantity quantity in ElementQuantity.Quantities.OfType<IIfcPhysicalQuantity>())
            {
                ...
                {
                    //CollectionProperty.Add(
                        yield return new EditorQuantity2x3(quantityArea, ModelIFC, this);
                }
                else if (quantity is IfcQuantityCount quantityCount)
                {
                    //CollectionProperty.Add(
                        yield return new EditorQuantity2x3(quantityCount, ModelIFC, this);
                }
                else if (quantity is IfcQuantityLength quantityLength)
                {
                    //CollectionProperty.Add(
                    yield return new EditorQuantity2x3(quantityLength, ModelIFC, this);
                }
                else if (quantity is IfcQuantityTime quantityTime)
                {
                    //CollectionProperty.Add(
                    yield return new EditorQuantity2x3(quantityTime, ModelIFC, this);
                }
                else if (quantity is IfcQuantityVolume quantityVolume)
                {
                    //CollectionProperty.Add(
                    yield return new EditorQuantity2x3(quantityVolume, ModelIFC, this);
                }
                else if (quantity is IfcQuantityWeight quantityWeight)
                {
                    //CollectionProperty.Add(
                    yield return new EditorQuantity2x3(quantityWeight, ModelIFC, this);
                }
            }
